...
Come si e' visto nei risultati \ref{risultati.alternativi} la formulazione matematica proposta nella sezione \ref{sec.formulazione2} non e' in grado di produrre soluzioni con un numero di sprint maggiore di 1, a causa della mancanza di vincoli che limitino la dimensione dello sprint e il rischio e l'incertezza massima che possono essere presenti in un unico sprint.
...
Una formiglore potrebbe essere la seguente:

\begin{alignat}{3}
 \qquad z_P  = \max  & \sum_{k=1}^m \sum_{i=1}^{k} \sum_{j=1}^{n} u_j \left( r_j^{cr} x_{ij} + a_j y_{ij} \right)
                                                                               &\                        \label{Eq6.P1}\\
  s.t.\ & \sum_{j=1}^{n} p_j r_j^{un} x_{ij} - p^{sp} \xi_{i}  \leq 0,                   &\ i \in S                \label{Eq6.P2}\\
        & \sum_{j=1}^{n} r_j^{risk} x_{ij} \approx \sum_{j=1}^{n} r_j^{risk} x_{(i+1)j},                   &\ i \in S                \label{Eq6.P3}\\
