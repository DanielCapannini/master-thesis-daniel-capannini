...
I metodi di \emph{cutting plane} costituiscono una tecnica fondamentale nella programmazione lineare intera mista (MILP), utilizzata per rafforzare iterativamente il rilassamento lineare continuo aggiungendo disuguaglianze valide (\emph{tagli}) che eliminano soluzioni frazionarie non fattibili per il problema discreto. Nel contesto del problema di assegnazione sprint-user story, i tagli di precedenza OR/AND impongono che, per eseguire una user story $j$ nello sprint $i$, almeno una (OR) o tutte (AND) le sue dipendenze predecessorali siano state completate negli sprint precedenti ($i_1 \leq i$).

Per potenziare la componente di branch-and-cut dell'euristica matmaster, sono stati sostanzialmente rivisti due metodi fondamentali: la funzione di setup \texttt{CuttingPlaneHeu} (ora \texttt{CuttingPlaneHeu\_new}) e l'intero sistema di generazione di tagli di precedenza.

\subsection{Limitazioni della versione originale}
L'implementazione precedente soffriva di:
\begin{itemize}
    \item \textbf{Parametri di debug invasivi}: output eccessivo che rallentava l'esecuzione
    \item \textbf{Generazione di un solo taglio per callback}: inefficace per problemi con molte violazioni
    \item \textbf{Bug critico negli indici}: \texttt{OR\_AND\_InequalitiesHeu} usava indici lineari $x_j$ invece di $x_{i,j}$, generando tagli errati
    \item \textbf{Nessun controllo della profondità}: tagli generati a tutti i livelli dell'albero
    \item \textbf{Nessuna deduplicazione}: rischio di rigenerare gli stessi tagli
\end{itemize}

\subsection{Miglioramenti introdotti}
La nuova versione implementa:
\begin{enumerate}
    \item \textbf{Limitazione alla profondità dell'albero}: tagli solo per $depth \leq 3$
    \item \textbf{Generazione multi-cut}: fino a 10 tagli per callback
    \item \textbf{Deduplicazione via FNV-1a}: checksum a 64 bit sulle colonne del taglio
    \item \textbf{Tolleranza ridotta}: $\epsilon = 10^{-6}$ invece di $10^{-3}$
    \item \textbf{Indici corretti}: iterazione su tutti gli sprint $i_1 \leq i$
    \item \textbf{Gestione CPLEX avanzata}: \texttt{CPX\_USECUT\_FILTER} per auto-purge
\end{enumerate}

\begin{figure}[h]
\begin{lstlisting}[style=pseudo, caption={myHeucutcallbackNew: algoritmo multi-cut},label={Alg.NewCallback}]
myHeucutcallbackNew(env, cbdata, wherefrom, cbhandle):
    if FlagLP or depth > 3: return
    
    Get node solution x[0..numcols-1]
    
    numCutsAdded = 0
    FOR iter = 1 to 10:
        if NOT OR_AND_InequalitiesNew(cutinfo): break
...
La funzione \texttt{OR\_AND\_InequalitiesNew} ora genera correttamente i tagli di precedenza considerando la dimensione temporale:

\begin{figure}[h]
\begin{lstlisting}[style=pseudo, caption={OR-AND-InequalitiesNew: tagli corretti},label={Alg.NewCuts}]
OR_AND_InequalitiesNew(cutinfo):
    FOR j in user stories:
        FOR i in sprints:
            if x[i*n+j] < 1e-9: continue
            
            // OR precedence: almeno UNA dipendenza prima
            lhs = sum_{i1<=i, u in UOR[j]} x[i1*n+u]
            if lhs - x[i*n+j] > 1e-6:
                Build cut: sum_{i1<=i,u in UOR[j]} x[i1*n+u] - x[i*n+j] <= 0
                if checksum != last_cut_checksum:
                    return VIOLATED
            
            // AND precedence: TUTTE le dipendenze prima
            lhs = sum_{i1<=i, u in UAND[j]} x[i1*n+u]
            if lhs - |UAND[j]|*x[i*n+j] > 1e-6:
                Build cut: sum_{i1<=i,u in UAND[j]} x[i1*n+u] - |UAND[j]|*x[i*n+j] <= 0
                if checksum != last_cut_checksum:
                    return VIOLATED
    return NO_VIOLATION
\end{lstlisting}
...
Questi miglioramenti rendono il branch-and-cut significativamente più robusto ed efficace, specialmente per istanze con catene di dipendenze complesse.



\section{Funzioni obbiettivo}\label{sec:fo}

Per valutare approcci alternativi alla funzione obbiettivo originale~\eqref{Eq1.P1}, sono state introdotte cinque nuove funzioni obbiettivo, progettate per catturare diversi aspetti critici della pianificazione degli sprint:
...
\begin{itemize}
    \item \(dp_{ij}\): \textit{development penalty} (0.25--1.0), misura il completamento delle dipendenze di $j$ prima dello sprint $i$. Assume valore 1.0 se tutte le dipendenze sono completate, 0.25 se nessuna lo è.
    
    \item \(ub_{ij}\): \textit{urgency bonus} ($\geq 0$), bonus proporzionale al numero di user story che dipendono da $j$ nello sprint $i$. Vale 0 se $j$ non ha dipendenze in ingresso.
\end{itemize}

\subsection{Caratteristiche delle funzioni obbiettivo}

Le funzioni \eqref{Eq2.P2}--\eqref{Eq2.P5} incorporano il parametro \(r_j^{unc}\), che quantifica l'\textit{incertezza} nelle stime di sviluppo di $j$. Questo termine privilegia l'assegnazione precoce delle user story più incerte, garantendo tempo sufficiente per gestire eventuali problemi.

Le funzioni \eqref{Eq2.P4}--\eqref{Eq2.P5} utilizzano invece \(p_j\), il peso di $j$, per massimizzare il \textit{carico di lavoro} degli sprint, riducendo lo spreco di capacità e accelerando il completamento del progetto.

\subsection{Ruolo dei parametri di modifica}

Ogni famiglia di funzioni obbiettivo è declinata in due varianti:
\begin{itemize}
    \item \textbf{Senza modificatori} (~\eqref{Eq2.P2},~\eqref{Eq2.P4}): versione base.
    \item \textbf{Con modificatori} (~\eqref{Eq2.P1},~\eqref{Eq2.P3},~\eqref{Eq2.P5}): include \(dp_{ij}\) e \(ub_{ij}\).
\end{itemize}

I modificatori perseguono due obiettivi principali:
\begin{enumerate}
    \item Prioritizzare nelle fasi iniziali le user story ``radice'' (senza dipendenze in ingresso o con molte dipendenze in uscita).
    \item Migliorare la convergenza algoritmica riducendo le violazioni di dipendenza che attivano i tagli.
\end{enumerate}
...
\subsection{Generazione nuovi dati}

purtroppo i dati che sono stati utilizzati per la valutazione sperimentale degli algoritmi proposti in \cite{BOSCHETTI2014116} presentano alcune limitazioni, per quanto riguarda i parametri riguardanti il rischio e l'incertezza delle user story essendo che non erano stati presi in considerazione. 
Percio, per poter valutare al meglio l'efficacia delle nuove funzioni obiettivo introdotte nel Capitolo 4, e' stato generato un nuovo set di dati sintetici i quali sono stati generati tramite un algoritmo che permette di una volte settati i parametri seguenti: 

\begin{itemize}
    \item numero di user story;
    \item numero di sprint;
    \item utilita minima e massima delle user story;
    \item rischio minimo e massimo delle user story;
    \item incertezza minima e massima delle user story;
    \item story point minimo e massimo delle user story;
    \item capacita minima e massima degli sprint;
    \item probabilità che una user story abbia vincoli di dipendenza;
    \item numero massimo di vincoli di dipendenza per user story.
\end{itemize}

L'algoritmo genera in modo casuale le user story e i relativi parametri, e successivamente genera i vincoli di dipendenza tra le user story in base alla probabilità impostata.

Grazie a questo algoritmo sono stati generati 20 nuovi progetti sintetici suddivi in 4 gruppi da 5 progetti ciascuno, i quali presentano le seguenti caratteristiche:

\begin{itemize}
  \item \textbf{Gruppo 1}: le istanze sono formate da 30 user story e 6 sprint e massimo 3 vincoli di dipendenza per user story;
  \item \textbf{Gruppo 2}: le istanze sono formate da 70 user story e 20 sprint e massimo 3 vincoli di dipendenza per user story;
  \item \textbf{Gruppo 3}: le istanze sono formate da 150 user story e 40 sprint e massimo 4 vincoli di dipendenza per user story;
  \item \textbf{Gruppo 4}: le istanze sono formate da 350 user story e 100 sprint e massimo 4 vincoli di dipendenza per user story;
\end{itemize}

Le caratteristiche principali delle istanze generate sono riportate nella Tabella~\ref{Tab.newData}.
...
instance\_06\_01 &  &  25 & 16 & 11 & 2 & 3 & 3 & 3 \\
instance\_06\_01 &  &  25 & 16 & 11 & 3 & 5 & 3 & 4 \\
instance\_06\_01 &  &  25 & 16 & 12 & 2 & 6 & 3 & 5 \\
instance\_06\_01 &  &  25 & 16 & 13 & 3 & 3 & 3 & 3 \\
instance\_06\_01 &  &  25 & 16 & 13 & 3 & 5 & 3 & 4 \\
\midrule

\multicolumn{9}{c}{\textbf{Gruppo 2}} \\
instance\_06\_01 &  &  60 & 40 & 14 & 9 & 9 & 3 & 6 \\
instance\_06\_01 &  &  60 & 40 & 14 & 6 & 10 & 3 & 6 \\
instance\_06\_01 &  &  60 & 40 & 14 & 7 & 8 & 3 & 4 \\
instance\_06\_01 &  &  60 & 40 & 11 & 5 & 7 & 3 & 5 \\
instance\_06\_01 &  &  60 & 40 & 17 & 5 & 7 & 3 & 3 \\
\midrule

\multicolumn{9}{c}{\textbf{Gruppo 3}} \\
instance\_06\_01 &  &  120 & 80 & 14 & 18 & 12 & 2 & 3 \\
instance\_06\_01 &  &  120 & 80 & 17 & 12 & 19 & 2 & 5 \\
instance\_06\_01 &  &  120 & 80 & 16 & 13 & 22 & 2 & 3 \\
instance\_06\_01 &  &  120 & 80 & 14 & 16 & 25 & 2 & 5 \\
instance\_06\_01 &  &  120 & 80 & 12 & 21 & 18 & 2 & 4 \\
\midrule

\multicolumn{9}{c}{\textbf{Gruppo 4}} \\
instance\_06\_01 &  &  300 & 250 & 16 & 41 & 42 & 2 & 6 \\
instance\_06\_01 &  &  300 & 250 & 17 & 48 & 52 & 2 & 4 \\
instance\_06\_01 &  &  300 & 250 & 16 & 50 & 46 & 2 & 5 \\
instance\_06\_01 &  &  300 & 250 & 17 & 42 & 58 & 2 & 5 \\
instance\_06\_01 &  &  300 & 250 & 12 & 40 & 52 & 2 & 5 \\
\midrule
\end{tabular}
\end{table}
...
Per valutare le prestazioni degli algoritmi e delle funzioni obbiettivo proposte sono stati ripresi dei KPI (Key Performance Indicators) che erano stati definiti in \cite{BOSCHETTI2014116} per valutare l'efficacia dei vari algoritmi implementati.
Nello specifico, i KPI considerati sono i seguenti:

\begin{description}
    \item[\(Z_{OPT}\)] Valore della soluzione ottima dal modello di base risolto da IBM ILOG CPLEX.
    
    \item[\(Z_{HEU}\)] Valore della migliore soluzione euristiche trovata.
    
    \item[\textbf{Gap}] Gap percentuale tra \(Z_{HEU}\) e il bound superiore del miglior nodo rimanente:
    \[
    \text{Gap} = 100 \times \frac{Z_{OPT}^{ub} - Z_{HEU}}{Z_{OPT}^{ub}}
    \]
    
    \item[\textbf{Nodes}] Numero di nodi esplorati nell'albero di branch-and-bound.
    
    \item[\textbf{NCuts}] Numero totale di tagli generati dalle callback:
    \begin{itemize}
        \item Vincoli di precedenza~\eqref{Eq1.P4}--\eqref{Eq1.P5} (lazy constraints)
        \item Disuguaglianze di Dominanza (DIs)
        \item LCIs (Logic-based Cutting Inequalities)
    \end{itemize}
    
    \item[\textbf{GHeu}] Gap percentuale qualità soluzione euristiche:
    \[
    G_{heu} = 100 \times \frac{Z_{OPT} - Z_{HEU}}{Z_{OPT}}
    \]
    
    \item[\textbf{Time}] Tempo computazionale totale (secondi).
\end{description}

A questi KPI sono stato aggiunti altri indicatori per valutare la bonta delle soluzioni proposte, a questo scopo sono stai introdotti i seguenti KPI:

\begin{description}
    \item[\textbf{N.S.O.} (Numero Sprint Occupati)] Numero di sprint utilizzati nella soluzione, che misura l'efficienza temporale della pianificazione: minori sprint occupati implicano consegna più rapida del progetto.
    
    \item[\textbf{Avg.S.U.} (Media Utilizzo Sprint)] Percentuale media di capacità sfruttata negli sprint occupati, che valuta l'efficienza di allocazione delle risorse: valori vicini al 100\% indicano minimizzazione dello spreco di capacità.
    
    \item[\textbf{Dev.Risk}] Deviazione standard del rischio~\(r_j^{cr}\) delle user story negli sprint, che quantifica la distribuzione equilibrata del rischio: bassa deviazione evita sprint ``troppo rischiosi'' o ``troppo sicuri''.
    
    \item[\textbf{Dev.Unc.}] Deviazione standard dell'incertezza~\(r_j^{unc}\) delle user story negli sprint, che misura la distribuzione equilibrata dell'incertezza: bassa variabilità favorisce gestione prevedibile degli sprint.
    
    \item[\textbf{Min.Risk}] Rischio minimo tra tutti gli sprint occupati, che verifica l'assenza di sprint ``eccessivamente sicuri'' evitando sottoutilizzo delle capacità di gestione del rischio del team.
    
    \item[\textbf{Min.Unc.}] Incertezza minima tra tutti gli sprint occupati, che garantisce la presenza di user story con incertezza significativa in ogni sprint, evitando sprint ``banali''.
    
    \item[\textbf{Max.Risk}] Rischio massimo tra tutti gli sprint occupati, che monitora l'esposizione al rischio nel worst-case: valori eccessivi segnalano sprint critici potenzialmente ingestibili.
    
    \item[\textbf{Max.Unc.}] Incertezza massima tra tutti gli sprint occupati, che identifica l'esposizione all'incertezza nel worst-case, essenziale per valutare la robustezza della pianificazione.
    
    \item[\textbf{Half.U.S.} (Half Utility Sprint)] Sprint in cui si raggiunge il 50\% del valore totale di utilità~\(U = \sum_j u_j\), che misura la progressione del valore consegnato (time-to-half-value): indica quanto rapidamente il progetto delivers valore al cliente.
\end{description}

I KPI computazionali valutano la qualità dell'ottimizzazione, mentre i nuovi KPI misurano l'idoneità pratica delle soluzioni per la pianificazione Agile reale, bilanciando qualità tecnica e valore business.
