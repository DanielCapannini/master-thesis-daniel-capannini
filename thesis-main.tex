...
\end{itemize}
Per queste valutazioni si è scelto di utilizzare per tutti i test la funzione obbiettivo \eqref{Eq1.P1} e per cui le considerazioni che seguono si dasano sul confronto tra i risultati che ottenuti modificando i parametri delle istanze e quelli ottenuti con i parametri originali riportati nelle tabelle \ref{tab.new1}, \ref{tab.new2} e \ref{tab.new02}.
...
\section{Confronto funzioni obbiettivo}\label{confronto:fb}

In questa sezione vengono confrontati i risultati ottenuti utilizzando le due diverse funzioni obbiettivo descritte nella sezione \ref{sec:fo}. Si usera come riferimento la tabella \ref{tab.new1} la quale riporta i risultati ottenuti con la funzione obbiettivo \eqref{Eq1.P1} e nelle tabelle \ref{tab.new2}, \ref{tab.new02} che riportano i risultati delle metriche calcolate su tali risultati.
...
\subsection{funzuone obbiettovo \texorpdfstring{\eqref{Eq2.P1}}{(5.23)}}

Nella tabella \ref{tab.new3} vi e' riportato il confronto tra la funzione obbiettivo \eqref{Eq1.P1} e la funzione obbiettivo \eqref{Eq2.P1} in termini di tempo di esecuzione. come si pou notare vi differenze tra i tempi di esecuzione delle 2 funzioni obbiettivo, non considerando CPLEX che come gia detto tende a saturare il tempo massimo di esecuzione, i tempi di esecuzione del algoritmo euristico con la funzione obbiettivo \eqref{Eq2.P1} sono il $16.3\%$ piu veloci rispetto quelli della funzione obbiettivo \eqref{Eq1.P1}, mentre i tempi di esecuzione dell'euristica lagrangiana variano soli di un $2.2\%$ sempre a favore della funzione obbiettivo\eqref{Eq2.P1}. 
Per quanto riguarda i KPI delle due funzioni obbiettivo si possono notare delle differenze minime, tra le tabelle \ref{tab.new2} e \ref{tab.new03} si possono notare che vi sono alcune istanze per cui i KPI N.U.S. e H.U.S. risultano inferiri per le istanze delle funzione obbiettivo \eqref{Eq2.P1}, percio questo indica che l'anticipare se possibile la risoluzione dei vincoli porta poi successivamente a veloccizzare il progetto.
Si puo anche notare confrontando le tabelle \ref{tab.new02} e \ref{tab.new04} che i valori di D.R. e R.MAX risultano essere leggermente maggiori per la funzione obbiettivo \eqref{Eq1.P1}, rispettivamente D.R. della funzione obbiettivo \eqref{Eq1.P1} e' $9.8\%$ maggiore rispettoa quello della funzione obbiettivo \eqref{Eq2.P1} metre per  R.MAX e' maggiore del $ 13.2\%$, questo indica che anticipare la risoluzione dei vincoli porta ad avere sprint mediamente meno rischiosi.
...
\subsection{Funzione obbiettivo \texorpdfstring{\eqref{Eq2.P2}}{(5.24)} e \texorpdfstring{\eqref{Eq2.P3}}{(5.25)}}

I risultati ottenuti utilizzando le funzioni obbiettivo \eqref{Eq2.P2} e \eqref{Eq2.P3} come riportato nella tabella \ref{tab.new4}, che indica i tempi di esecuzione delle 2 funzioni obbiettivo, come nella relazione tra le funzioni obbiettivo \eqref{Eq1.P1} e \eqref{Eq2.P1}, vi e' un discostamento a per quanto riguarda i tempi di esecuzione dell'algoritmo euristico in cui la funzione obbiettivo \eqref{Eq2.P3} risulta piu veloce del $15.1\%$ rispetto alla sua controparte \eqref{Eq2.P2}, mentre per quanto riguarda l'euristica lagrangiana i tempi di esecuzione non si discostano in modo significativo. 
Al contrario di quello che si e' notato nel confronto tra le funzioni obbiettivo \eqref{Eq1.P1} e \eqref{Eq2.P1}, in questo caso il confronto dei KPI N.U.S. e H.U.S. tra le due funzioni obbiettivo risulta risulta per alcune istanze favorevole alla funzione obbiettivo \eqref{Eq2.P2} che risulta riuscire a concludere le istanze con un numero leggermente minore di sprint rispetto alla funzione obbiettivo \eqref{Eq2.P3}.
Per quanto riguarda i KPI D.U. e U.MAX si puo notare confrontando le tabelle \ref{tab.new004} e \ref{tab.new005} che la funzione obbiettivo \eqref{Eq2.P2} rusulta avere un valore del D.U. del $ 8.1\%$ maggiore rispetto a quello della funzione obbiettivo \eqref{Eq2.P3}, mentre per il KPI U.MAX risulta essere maggiore del $ 12.3\%$, questo indica che vi sono presenti in media sprint maggiormente incerti. 
...
\subsection{Funzione obbiettivo \texorpdfstring{\eqref{Eq2.P4}}{(5.26)} e \texorpdfstring{\eqref{Eq2.P5}}{(5.27)}}

I risultati ottenuti risolvendo le istanze con le funzioni obbiettivo \eqref{Eq2.P4} e \eqref{Eq2.P5} per quanto riguarda i tempi di esecuzione come mostrato nella tabella \ref{tab.new6} risulta esserci una differenza che mostra che la funzione obbiettivo \eqref{Eq2.P5} risulta essere piu veloce del $14.6\%$ se si usa il metodo euristoco e del $16.4\%$ se si usa l'euristica Lagrangiana rispetto alla funzione obbiettivo \eqref{Eq2.P4}.
Osservando i KPI nelle tabelle \ref{tab.new06} e \ref{tab.new07} si nota una differenza in alcune istanze sostanzile per quanto riguarda il valore di H.U.S. che risulta essere piu basso utilizzando la funzione obbiettivo \eqref{Eq2.P4} rispetto alla funzione obbiettivo \eqref{Eq2.P5}, mentre il valore di N.U.S. rusilta essere pressoche uguale, questo sta ad indicare che per questa tipologia di funzione obbiettivo considerare i parametri $dp_{ij}$ e $ub_{ij}$ a un risultato non efficente dal punto di vista dell'utilita dell'utente.
Osservando invece le tabelle \ref{tab.new006} e \ref{tab.new007} si nota come i valori di D.R., D.U. R.MAX e U.MAX risultano essere piu bassi utilizzando la funzione obbiettivo \eqref{Eq2.P5} di un valore compreso tra $19.8\%$ e $21.5\%$ rispetto alla funzione obbiettivo \eqref{Eq2.P4}, questo indica che la funzione obbiettivo \eqref{Eq2.P5} riesce a distribuire meglio sia il rischio che l'incertezza lungo i vari sprint rispetto alla funzione obbiettivo \eqref{Eq2.P4}.
...
\subsection{Confronto funzioni obbiettivo}

In questa sezione si confrontano i risultati presentati precedentemente delle diverse funzioni obbiettivo considerando solo il metodo euristico.

Come si puo osservare nella figura \ref{fig:tempoFO}, tutte le funzioni obbiettivo presentano tempi di calcolo simili per quanto riguara i primi tre gruppi di istanze (es. instance\_06\_xx, instance\_20\_xx, instance\_40\_xx). Per le istanze piu grandi del gruppo 4 e quelle che presentano story point in base alle sequenza di Fibonacci (es. instance\_100\_xx e instance\_f\_xx) si nota che le funzioni obbiettivo \eqref{Eq2.P4} e \eqref{Eq2.P5} impiegano piu tempo per essere risolte ed e' visibile il grap che e' stato presentato prima tra la funzione obbiettivo \eqref{Eq1.P1} e \eqref{Eq2.P1}, la funzione obbiettivo \eqref{Eq2.P2} e \eqref{Eq2.P3} .
Questa differenza di tempi tra le funzioni obbiettivo \eqref{Eq2.P4} e \eqref{Eq2.P5} e le altre e' probabilmente dovuta al fatto che gli algoritmi risultano in diffilcolta a selezionare la combinazione di user story migliori confrontando la quantita di story point che richiedono.


\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/tempoFO1.png}
    \caption{Tempo di calcolo per le diverse funzioni obbiettivo}
    \label{fig:tempoFO}
\end{figure}

Analizzando la figura \ref{fig:POSFO} si puo notare che la percentuale di utilizzo degli sprint risulta essere similare per tutte le funzioni obbiettivo, la funzione obbiettivo \eqref{Eq2.P5} risulta essere quella che si discosta piu spesso dalle altre ma solo per alcune istanze, il fatto che lo scostamento sia solo su certe istanze sta ad indicare che per quelle istanze in cui riesce a presentare una percentuale miglire rispetto alle altre e' dovuto al fatto che riesce a ad utilizzare uno sprint in meno e quindi rispetto agli altri non presentare uno sprint semi vuoto che abbassa la percentuale di utilizzo degli sprint. migliora questo paragrafo
...
nelle figure \ref{fig:dr}, \ref{fig:rmax}, \ref{fig:du}, \ref{fig:umax} si puo notare che in tutte e 4 le funzioni obbiettivo \eqref{Eq1.P1}, \eqref{Eq2.P2} e \eqref{Eq2.P4} risulatno presentare valori simili tra loro e maggiori rispetto alle altre funzioni obbiettivo, mentre la funzione obbiettivo \eqref{Eq2.P5} risulta essere quella che presenta i valori piu bassi in tutte e 4 le metriche considerate. Questo indica che la funzione obbiettivo \eqref{Eq2.P5} e' quella che riesce a bilanciare meglio il rischio e l'incertezza tra gli sprint rispetto alle altre funzioni obbiettivo e che le funzioni obbiettivo come la \eqref{Eq2.P1}, \eqref{Eq2.P3} e \eqref{Eq2.P5} che presentano al loro interno i parametri $dp_{ij}$ e $ub_{ij}$ risultano presentare valori minori rispetto alle controparti senza quindi riescono a distribuire meglio i rischi e le incertezze all'interno dei vari sprint. migliora questo paragrafo


\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/dr2.png}
    \caption{Deviazione standard del rischio per le diverse funzioni obbiettivo}
    \label{fig:dr}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Valore massimo del rischio per le diverse funzioni obbiettivo}
    \label{fig:rmax}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Deviazione standard dell'incertezza per le diverse funzioni obbiettivo}
    \label{fig:du}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Valore massimo dell'incertezza per le diverse funzioni obbiettivo}
    \label{fig:umax}
\end{figure}
...
Per queste valutazioni si è scelto di utilizzare per tutti i test la funzione obbiettivo \eqref{Eq1.P1} e per cui le considerazioni che seguono si basano sul confronto tra i risultati ottenuti modificando i parametri delle istanze e quelli ottenuti con i parametri originali riportati nelle tabelle \ref{tab.new1}, \ref{tab.new2} e \ref{tab.new02}.

\subsection{Riduzione del numero di sprint}

La riduzione del numero di sprint disponibili indica una situazione in cui il team di sviluppo ha meno tempo a disposizione per completare il progetto, il che puo portare a una maggiore pressione e a una maggiore difficoltà nel completare tutte le user story entro il tempo previsto se non all'impossibilita di completarlo.
Nel nostro test è importante valutare come la riduzione del numero di sprint influisce sulla qualità della soluzione e sui tempi di calcolo. I test che sono stati dondotti hanno previsto una riduzione di 2 sprint rispetto al numero di sprint originale per ogni istanza, e i risultati ottenuti sono riportati nelle tabelle \ref{tab.new8}, \ref{tab.new08} e \ref{tab.new008}.
Si possono notare che le istanze $instance\_06\_02$, $instance\_06\_03$ e $instance\_100\_05$ presentano una soluzione con un valore della funzione obbiettivo pari a $Zopt = 0$ per il metodo esatto, questo indica che per quelle istanze non si puo avere una soluzione fattibile riducendo il numero di sprint a disposizione, metre le le altre istanze si puo notare che la riduzione del numero di sprint a disposizione non ha avuto nessun impatto sulla soluzione ottenuta dai metodi euristici.
Si puo pero notare una variazione nei tempi di calcolo, per il metodo esatto la riduzione del numero di sprint porta un rispormi del $61.01\%$ di tempo di calcolo, metre per il metodo euristico del $117.54\%$ e per il metodo euristico Lagrangiano del $162.14\%$. Da questultima considerazione si puo notare come la riduzione del numero di sprint a disposizione porta a una diminuzione dei tempi di calcolo. 
...
In un progetto reale, e' possibile che la capacita degli sprint venga ridotta a causa di imprevisti, come ad esempio malattie, ferie o altri impegni dei membri del team, questo porta a un aumento dei tempi di completamento del progetto e in alcuni casi l'impossibilita al completamento nei tempi previsti.
Per valutare l'impatto di una riduzione della capacita degli sprint sui KPI, abbiamo condotto un esperimento in cui abbiamo ridotto la capacita degli sprint del 20\% i cui risultati si trovano nelle tabelle \ref{tab.new9}, \ref{tab.new09} e \ref{tab.new009}.
Si puo  notare come la riduzione della capacita dello sprint ha portato anche a una riduzione del valore della funzione obbiettivo del $8\%$ per il metodo esatto e del $6.1\%$ per i metodi euristici e si nota anche cle il gruppo di istanze piu grandi non e' in grado di essere risolto causa il non sufficenti story point a disposizione.
Il KPI N.U.S. e' aumentato del $26.59\%$ per il metodo esatto e del $25.08\%$ per i metodi euristici, mentre per il KPI H.U.S. vi e' stato un aumentato del $26.67\%$ per il metodo esatto e del $15.51\%$ per i metodi euristici. Questo indicando come era presumibile che la riduzione della capacita degli sprint ha portato ad un aumeto del numero degli sprint necessari a completare il progetto.
...
In questo modo, si ottimizza il flusso di lavoro mantenendo flessibilità.
Per i test computazionali, si è deciso di accorpare gli sprint in gruppi di 2 a partire dalla metà del numero totale di sprint. 
...
Si puo notare che l'accorpamento degli sprint non ha impattato sul valore ottenuto dalla  funzione obbiettivo rispetto alla versione base, questo dovuto al fatto che l'accorpamento non ha modificato la capacità totale disponibile, ma solo la distribuzione della stessa tra gli sprint. 
Vi sono pero differenze sui KPI D.R. e D.U. che hanno avuto un aumento, per quanto ruguarda D.R. per la soluzione esatta l'aumento e' stato del $2.59\%$ mentre per le soluzioni euristiche e' stato del $8.83\%$, per quanto riguarda D.U. per la soluzione esatta l'aumento e' stato del $3.11\%$ mentre per le soluzioni euristiche e' stato del $11.36\%$.
Lo stesso trend si nota anche nei KPI R.MAX e U.MAX, con un aumento rispettivamente del $9.75\%$ e del $11.02\%$ per le soluzioni euristiche, mentre per la soluzione esatta l'aumento è stato del $3.01\%$ per R.MAX e del $3.46\%$ per U.MAX. Questo aumento e' dovuto dal fatto che vi sono sprint piu' grandi che permettono di allocare piu' lavoro in un singolo sprint, ma questo comporta anche un aumento del rischio e dell'incertezza complessiva dello sprint.
...
L'eliminazione dei vincoli AND e OR in un progetto reale non sarebbe possibile, visto che  risulta impossibile eliminare tutte le dipendenze tra le attivita. Tuttavia si e' voluto testare se questi vincoli incidono nulla soluzione e se si in quale misura. I risultati ottenuti sono riportati nelle tabelle \ref{tab.new11}, \ref{tab.new011} e \ref{tab.new0011}. 
Si puo notare che i risultati ottenuti risultano essere migliori rispetto a quelli ottenuti con i vincoli AND e OR, il valore delle funzione obbiettvo e' aumentoato del $42.65\%$ per il metodo esatto e del $41.91\%$ per i metodo euristici. I tempi di esecuzione sono
...
Il miglioramento del valre della funzione obbiettvo e' dovuto al fatto che l'eliminazione dei vincoli permette di avere una maggiore flessibilita nella pianificazione delle attivita, consentendo di sfruttare meglio le risorse disponibili e di ridurre i tempi di completamento del progetto. Inoltre, l'eliminazione dei vincoli AND e OR consente di ridurre 
il tempo di esecuzione perche una qualsiasi soluzione che soddisfa i vincoli sulla capcita degli sprint e' una soluzione ammissibile per cui non saranno necessari cercARE ULTERIORI SOLUZIONI.
